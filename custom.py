...
def compute_bonus():
    ...
    try:
        ...
        # bonus = 0
        bonus = user_data['data'][-3]['trialdata'][1] # Is the regualar bonus
        additionalBonus = user_data['data'][-1]['trialdata'][1] #is the bonus from the bonus questionnaire
        user.bonus = bonus + additionalBonus
        db_session.add(user)
        db_session.commit()
